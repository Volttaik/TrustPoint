...
function BankDot({ name, size = 22 }: { name: string; size?: number }) {
  ...
  return (
    <View style={{ width: size, height: size, borderRadius: size / 2,
      ...
      <Text style={{ fontSize: size * 0.38, color: "#fff", fontFamily: "Inter_700Bold" }}>{lbl}</Text>
    </View>
  );
}

/* ─── Info-circle icon ───────────────────────────────────── */
function InfoIcon({ size = 13, color }: { size?: number; color: string }) {
  return (
...
export default function ReviewScreen() {
  ...
  const Row = ({
    ...
    bold?: boolean; valueColor?: string; infoTip?: boolean;
  }) => (
    <View style={styles.feeRow}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 5 }}>
        <Text style={[
          styles.feeLabel,
          { color: bold ? colors.text : colors.mutedForeground,
            fontFamily: bold ? "Inter_700Bold" : "Inter_400Regular" },
        ]}>
          {label}
        </Text>
        {infoTip && <InfoIcon color={colors.mutedForeground} />}
      </View>
      <Text style={[
        styles.feeValue,
    ...
    <View style={[styles.root, { backgroundColor: colors.background, paddingTop: topPad }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <TouchableOpacity
          ...
          hitSlop={8}
        >
          <TpIcon name="x" size={18} color={colors.text} strokeWidth={2} />
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <View style={styles.hero}>
          <Text style={[styles.heroAmount, { color: colors.text, fontFamily: "Inter_700Bold" }]}>
            {fmt(numAmount)}
          </Text>
          <Text style={[styles.heroWords, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>
            {numToWords(numAmount)}
          </Text>
          {cashback > 0 && (
            <View style={[styles.cashbackPill, {
              ...
              <Text style={[styles.cashbackTxt, { color: colors.primary, fontFamily: "Inter_600SemiBold" }]}>
                ₦{cashback} cashback
              </Text>
            </View>
          )}
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={styles.recipientHeader}>
            <Text style={[styles.toName, { color: colors.text, fontFamily: "Inter_700Bold" }]}>
              To {recipientName}
            </Text>
            <View style={styles.recipientSub}>
              <BankDot name={recipientBank} size={20} />
              <Text style={[styles.recipientSubTxt, { color: colors.mutedForeground, fontFamily: "Inter_400Regular" }]}>
          ...
          <View style={styles.feeBlock}>
            ...
            {cashback > 0 && (
              <Row label="Cashback earned" value={`₦${cashback}`} valueColor={colors.primary} />
            )}
          </View>
        </View>
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={styles.acctRow}>
              <View style={[styles.acctAvatar, {
                ...
                <Text style={[styles.acctInitials, { color: colors.text, fontFamily: "Inter_700Bold" }]}>
        ...
        <View style={styles.section}>
          ...
          <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <TextInput
              ...
              style={[styles.narrationInput, { color: colors.text, fontFamily: "Inter_400Regular" }]}
              multiline
              maxLength={140}
              returnKeyType="done"
              blurOnSubmit
      ...
      <View style={[styles.cta, {
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.88}
        >
          <Text style={[styles.payTxt, { color: colors.primaryForeground, fontFamily: "Inter_700Bold" }]}>
            Pay {fmt(total)}
          </Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  scroll: { paddingHorizontal: 16, paddingTop: 22, gap: 16 },

  hero:         { alignItems: "center", gap: 7, paddingVertical: 8 },
  heroAmount:   { fontSize: 46, letterSpacing: -2 },
  heroWords:    { fontSize: 14 },
  cashbackPill: {
    borderWidth: 1, borderRadius: 20,
    paddingHorizontal: 14, paddingVertical: 5, marginTop: 4,
  },
  cashbackTxt: { fontSize: 13 },

  card: {
    borderRadius: 16, borderWidth: 1, overflow: "hidden",
  },

  recipientHeader: { padding: 16, gap: 7 },
  toName:          { fontSize: 16, letterSpacing: -0.3 },
  recipientSub:    { flexDirection: "row", alignItems: "center", gap: 7 },
  recipientSubTxt: { fontSize: 13 },

  sep: { height: StyleSheet.hairlineWidth },

  feeBlock: { paddingHorizontal: 16, paddingVertical: 6 },
  feeRow:   {
    flexDirection: "row", alignItems: "center",
  ...
  section:      { gap: 10 },
  sectionLabel: { fontSize: 13, paddingLeft: 2 },

  acctRow: { flexDirection: "row", alignItems: "center", gap: 12, padding: 14 },
  acctAvatar: {
    width: 42, height: 42, borderRadius: 10,
    borderWidth: 1, alignItems: "center", justifyContent: "center",
  },
  acctInitials: { fontSize: 14 },
  acctName:     { fontSize: 14, marginBottom: 3 },
  acctNum:      {},
  acctBalance:  { fontSize: 15 },

  narrationInput: {
    fontSize: 14, padding: 16, minHeight: 80, textAlignVertical: "top",
  },
  ...
  payTxt: { fontSize: 17, letterSpacing: -0.3 },
});
