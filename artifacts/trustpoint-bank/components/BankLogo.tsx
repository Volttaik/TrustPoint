/**
 * BankLogo — renders a bank's logo at the requested size.
 *
 * Banks with a real SVG from the brand zip are rendered via SvgXml on a
 * white pill background (width auto-scales to the SVG aspect ratio).
 * GTBank and UBA use hand-coded JSX SVG because their icon backgrounds
 * are already perfect circles. TrustPoint uses its transparent PNG.
 * Opay uses its PNG on a teal background. All others fall back to a
 * brand-color circle with initials.
 */

import React from "react";
import { Image, StyleSheet, Text, View } from "react-native";
import Svg, { Path, Rect } from "react-native-svg";
import { SvgXml } from "react-native-svg";
import { getBankInfo } from "@/constants/banks";
import {
  accessBankSvg,
  moniepointSvg,
  firstBankSvg,
  premiumTrustSvg,
  stanbicIbtcSvg,
} from "@/constants/bank-svgs";

interface BankLogoProps {
  bankName?: string | null;
  /** Controls the HEIGHT of the logo; width auto-scales for wordmarks */
  size?: number;
  /** Rounds the container to a circle for icon-style logos */
  circular?: boolean;
}

/** Aspect ratios (W:H) of each wordmark SVG so we can auto-size width */
const WORDMARK_RATIOS: Record<string, number> = {
  "Access Bank":       1000 / 340,  // viewBox 1000×1000 but content fills ~340px tall
  "Moniepoint":        615.94 / 152.98,
  "First Bank":        1000 / 340,
  "PremiumTrust Bank": 1437.25 / 311.03,
  "Stanbic IBTC":      1000 / 300,
};

export function BankLogo({ bankName, size = 40, circular = true }: BankLogoProps) {
  const bank = getBankInfo(bankName);

  // ── TrustPoint Bank ───────────────────────────────────────────────
  if (bank.name === "TrustPoint Bank") {
    return (
      <View style={[styles.wrap, { width: size, height: size, borderRadius: size / 2, backgroundColor: "transparent" }]}>
        <Image
          source={require("@/assets/images/icon_transparent.png")}
          style={{ width: size, height: size }}
          resizeMode="contain"
        />
      </View>
    );
  }

  // ── Opay (PNG logo) ───────────────────────────────────────────────
  if (bank.name === "Opay") {
    return (
      <View style={[styles.wrap, { width: size, height: size, borderRadius: circular ? size / 2 : size * 0.22, backgroundColor: "#09C99A", overflow: "hidden" }]}>
        <Image
          source={require("@/assets/images/opay-logo.png")}
          style={{ width: size, height: size }}
          resizeMode="cover"
        />
      </View>
    );
  }

  // ── GTBank — keep as hand-coded SVG (perfect circle icon) ─────────
  if (bank.name === "GTBank") {
    return (
      <Svg width={size} height={size} viewBox="0 0 581 588">
        <Rect x="0" y="0" width="581" height="588" rx={circular ? 290 : 80} fill="#D94F00" />
        <Rect x="330" y="114" width="149" height="149" rx="6" fill="#fff" />
        <Path d="M150.5 338.75v14.6h25.67v77.24h17.7v-77.83h23.9v-14.6l-67.27.6zM137.28 356.85v-14.6c-14.2-6.42-30.53-8.36-47.25 0-27.42 12.26-28.4 72.97-.97 84.84 15.75 7.98 48.6 4.28 51.91-2.73V376.3h-32.66v12.65h14.38v25.3c-47.25 22.96-56-87.37 14.59-57.4zM246.56 388.17h16.91c14 .59 17.7 27.05-.58 26.85h-16.33v-26.85zm0-37.16h16.91c14 2.53 8.95 23.35-.58 23.93h-16.33v-23.93zm-17.5-14.2v93h46.47c23.72-8.56 24.7-45.33 0-49.23 22.75-2.52 23.92-44.36-15.36-44.36l-31.11.59zM311.11 362.88v13.81c12.44-3.5 39.47-13.23 36.94 7.4-23.72-1.56-39.86 2.72-42.58 17.5-4.47 31.92 29.75 35.03 44.14 19.27v7.59h16.33v-56.04c-4.08-18.1-31.1-20.43-54.83-9.53zm13.8 36c3.7-4.87 15.56-4.68 23.53-2.34v12.26c-13.02 14-34.02 7.59-23.52-9.93zM399.78 379.42c4.28-4.67 27.03-13.43 30.53 0v49.42h18.27V376.5c-3.89-21.8-24.7-25.69-49.97-10.9v-8.17h-17.3v71.8h18.27l.2-49.81z"
          fill="#fff" />
      </Svg>
    );
  }

  // ── UBA — keep as hand-coded SVG (clean V-arrow icon) ────────────
  if (bank.name === "UBA") {
    return (
      <Svg width={size} height={size} viewBox="0 0 500 500">
        <Rect width="500" height="500" rx={circular ? 250 : 70} fill="#fff" />
        <Path d="M277.353 126.125C242.336 126.374 220.861 164.578 238.832 194.621L320.801 331.499L372.772 125.447L277.353 126.125Z" fill="#D42E12" />
        <Path d="M179.628 168.851L127.657 374.903L223.076 374.224C258.093 373.976 279.568 335.772 261.597 305.729L179.628 168.851Z" fill="#D42E12" />
      </Svg>
    );
  }

  // ── Wordmark banks — render real SVG from brand assets ────────────
  const wordmarkSvg: string | null =
    bank.name === "Access Bank"       ? accessBankSvg  :
    bank.name === "Moniepoint"        ? moniepointSvg  :
    bank.name === "First Bank"        ? firstBankSvg   :
    bank.name === "PremiumTrust Bank" ? premiumTrustSvg :
    bank.name === "Stanbic IBTC"      ? stanbicIbtcSvg  :
    null;

  if (wordmarkSvg) {
    // Render inside a circle — wordmarks are wide so scale them small to fit
    const ratio = WORDMARK_RATIOS[bank.name] ?? 3.5;
    // Inner SVG: fill horizontally at ~88% of diameter; height = width / ratio
    const innerW = size * 0.88;
    const innerH = innerW / ratio;
    return (
      <View style={[styles.wrap, styles.wordmarkWrap, { width: size, height: size, borderRadius: size / 2, backgroundColor: "#ffffff" }]}>
        <SvgXml xml={wordmarkSvg} width={innerW} height={innerH} />
      </View>
    );
  }

  // ── Generic fallback: brand-color circle with initials ────────────
  return (
    <View style={[styles.wrap, { width: size, height: size, borderRadius: circular ? size / 2 : size * 0.22, backgroundColor: bank.color }]}>
      <Text style={[styles.initText, { fontSize: size * 0.32, color: "#fff", fontFamily: "Inter_700Bold" }]}>
        {bank.initials}
      </Text>
    </View>
  );
}

const styles = StyleSheet.create({
  wrap: {
    alignItems: "center",
    justifyContent: "center",
    overflow: "hidden",
  },
  wordmarkWrap: {
    borderWidth: 1,
    borderColor: "rgba(0,0,0,0.07)",
  },
  initText: {
    includeFontPadding: false,
    textAlignVertical: "center",
  },
});
