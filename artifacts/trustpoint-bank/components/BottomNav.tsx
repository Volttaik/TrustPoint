...
type NavTab = { name: string; label: string; icon: TpIconName };
...
function TabBtn({ tab, isActive }: { tab: NavTab; isActive: boolean }) {
  ...
  return (
    ...
    >
      <Animated.View style={[styles.tabContent, aStyle]}>
        <View style={[styles.iconSlot, isActive && { backgroundColor: colors.primary + "17" }]}>
          <TpIcon
            name={tab.icon}
            size={22}
...
export function BottomNav() {
  ...
  return (
    ...
    >
      <NavBarShape width={screenW} isDark={isDark} extraH={bottomH} />
      <View style={[styles.bar, { paddingTop: BUMP_H }]}>
        {LEFT_TABS.map((t)  => <TabBtn key={t.name} tab={t} isActive={isActive(t.name)} />)}
        <LogoBtn />
        {RIGHT_TABS.map((t) => <TabBtn key={t.name} tab={t} isActive={isActive(t.name)} />)}
      </View>
    </View>
  );
}
